  ...
  return (
    <Flex justify="space-between" w="full" zIndex="100" className='bg-white/50 fixed backdrop-blur-md dark:bg-zinc-900/50 px-7 md:px-20 py-1.5 border-b border-zinc-200/50 dark:border-zinc-700/50'>
      ...
      { /* Desktop Nav ( Media Device ) */ }
        ...
        <Flex className='text-prm-size text-zinc-500' align="center" display={{ base: 'none', md: 'flex' }}>
          <FeaturesMenu/>
        ...
        <Collapse in={isOpen} animateOpacity>
            <Flex className='text-sm backdrop-blur-sm bg-white dark:bg-zinc-900' direction="column" h={"100vh"} p={"5"} mt="2" position="absolute" top={"10"} w={"full"} left={"0"} zIndex="10" >
              ...
              <Flex align={"center"} gap={"1"} py={"5"} >
                ...
                <a target='_blank' href="https://github.com/craftzmen" className='hover:opacity-50 w-full inline-flex gap-x-4 px-4 py-3 rounded-md bg-zinc-100 font-semibold dark:bg-zinc-800 dark:text-zinc-300' >
                  <GithubIcon/> Github
                </a>
              </Flex>
            </Flex>
        ...
        { /* Navigation right pannel elements */ }
          ...
          <Flex align={"center"} className='md:gap-4' >
            <Flex align={"center"} className='translate-x-4' >
              ...
              <IconButton className='dark:text-zinc-300 transition-all' variant={"unstyled"} icon={isOpen ? <CrossIcon /> : <MenuIcon />} onClick={toggleNavbar} display={{ base: 'block', md: 'none' }} />
            </Flex>
            <NavLink to="404" className="hidden md:flex px-7 py-2 rounded-full font-bold bg-black text-white dark:bg-zinc-600/50 hover:opacity-70 focus:bg-zinc-100 focus:text-zinc-500 text-sm">Join</NavLink>
